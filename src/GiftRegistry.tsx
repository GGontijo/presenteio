...
export default function GiftRegistry() {
  ...
  const [newGift, setNewGift] = useState({ name: '', image: '', description: '' })
  ...
  const addGift = (e: React.FormEvent) => {
    ...
    if (newGift.image && newGift.description) {
      ...
      setNewGift({ name: '', image: '', description: '' })
  ...
  return (
    <div className="w-screen h-screen bg-gradient-to-b from-gray-100 to-pink-200 text-gray-800 p-8">
      ...
      <Dialog open={isAddGiftModalOpen} onOpenChange={setIsAddGiftModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-black">Adicionar Novo Item</DialogTitle>
          </DialogHeader>
          <form onSubmit={addGift} className="space-y-4">
          <div>
              <Label className="text-black" htmlFor="giftImage">Nome do Item</Label>
              <Input
                id="itemName"
                value={newGift.name}
            ...
            <div>
              <Label className="text-black" htmlFor="giftImage">URL da Imagem</Label>
              <Input
                id="giftImage"
                value={newGift.image}
                onChange={(e) => setNewGift({ ...newGift, image: e.target.value })}
            ...
            <div>
              <Label className="text-black" htmlFor="giftDescription">Descrição</Label>
              <Textarea
                id="giftDescription"
